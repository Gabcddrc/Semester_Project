...
def dataLoader(tokenizer, data, batch_size):
	...
	questions_encoding = tokenizer.batch_encode_plus(
		data['abbreviated'], padding= True, truncation=True, max_length=MAX_LEN, return_tensors="pt")
	with tokenizer.as_target_tokenizer():
		answers_encoding = tokenizer.batch_encode_plus(
			data['sentences'].tolist(), padding= True, truncation=True, max_length=MAX_LEN, return_tensors="pt")
...
data = pd.read_csv(
	"/itet-stor/zhejiang/net_scratch/datasetv2/train.csv", index_col='Id').iloc[:LOAD_SIZE]
...
valid_data = pd.read_csv(
	"/itet-stor/zhejiang/net_scratch/datasetv2/valid.csv", index_col='Id').iloc[:2000]
...
for i in range(EPOCHS):
	...
	if(score>best_score):
		...
		torch.save(model.state_dict(),
				'/itet-stor/zhejiang/net_scratch/models/seq2seq_T5_512.model')
